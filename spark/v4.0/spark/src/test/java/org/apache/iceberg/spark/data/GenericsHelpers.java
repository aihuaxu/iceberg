...
public class GenericsHelpers {
  ...
  private static void assertEqualsUnsafe(Variant expected, VariantVal actual) {
    VariantMetadata actualMetadata = VariantMetadata.from(ByteBuffer.wrap(actual.getMetadata()).order(ByteOrder.LITTLE_ENDIAN));
    VariantTestUtil.assertEqual(expected.metadata(), actualMetadata);
    VariantTestUtil.assertEqual(expected.value(), VariantValue.from(actualMetadata, ByteBuffer.wrap(actual.getValue()).order(ByteOrder.LITTLE_ENDIAN)));
  }
